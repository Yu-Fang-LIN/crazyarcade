...
class Player(pygame.sprite.Sprite):
    ...
    def update1(self, pressed_keys):          
        ...
        if pressed_keys[K_LSHIFT] and self.at_center():
            if self.bomb_num > 0:
                bomb = Bomb(self.rect.center[0], self.rect.center[1], self)
...
def explosion(bomb, power): #爆炸後發生的事
    print('explosion!')
    down_explosion, up_explosion, right_explosion, left_explosion = False, False, False, False
    for obj in all_sprites:
        if (bomb.rect.center[0] - 18 <= obj.rect.center[0] <= bomb.rect.center[0] + 18 and bomb.rect.center[1] - 18 <= obj.rect.center[1] <= bomb.rect.center[1] + 18
            and type(obj) != Rock and type(obj) != Bomb and (obj not in Props) and (not down_explosion)):
            obj.kill()
            fall = random.random()
            if fall > 0.7: #掉落威力藥水
    ...
    for i in range(40, power*40 + 1, 40):
        # print(i,end=' ')
        print(i)
        for obj in all_sprites:
            if ((bomb.rect.center[0] - 18 + i <= obj.rect.center[0] <= bomb.rect.center[0] + 18 + i) and 
            (bomb.rect.center[1] - 18 <= obj.rect.center[1] <= bomb.rect.center[1] + 18) and (type(obj) != Rock) and (type(obj) != Bomb) and (obj not in Props) and (not right_explosion)):
                obj.kill()
                right_explosion = True
                fall = random.random()
                if fall > 0.7: #掉落威力藥水
                    morepower = MorePower(obj.rect.center[0],obj.rect.center[1])
                    all_sprites.add(morepower)
                    Props.add(morepower)
                    morepowers.add(morepower)
            elif (bomb.rect.center[0] - 18 - i <= obj.rect.center[0] <= bomb.rect.center[0] + 18 - i and bomb.rect.center[1] - 18 <= obj.rect.center[1] <= bomb.rect.center[1] + 18 
            and type(obj) != Rock and type(obj) != Bomb and (obj not in Props) and (not left_explosion)):
                obj.kill()
                left_explosion = True
                fall = random.random()
                if fall > 0.7: #掉落威力藥水
                    ...
                    Props.add(morepower)
                    morepowers.add(morepower)
            elif (bomb.rect.center[0] - 18 <= obj.rect.center[0] <= bomb.rect.center[0] + 18 and bomb.rect.center[1] - 18 + i <= obj.rect.center[1] <= bomb.rect.center[1] + 18 + i 
            and type(obj) != Rock and type(obj) != Bomb and (obj not in Props) and (not down_explosion)):
                obj.kill()
                down_explosion = True
                fall = random.random()
            ...
            and type(obj) != Rock and type(obj) != Bomb and (obj not in Props) and (not up_explosion)):
                obj.kill()
                up_explosion = True
                fall = random.random()
                if fall > 0.7: #掉落威力藥水
...
while running:
    ...
    #撿藥水
    if pygame.sprite.spritecollideany(player1, morepowers):
        player1.bomb_power += 1
        pass
    if pygame.sprite.spritecollideany(player2, morepowers):
        player2.bomb_power += 1        
        pass
    if pygame.sprite.groupcollide(players, Props, False, True):
        pass
